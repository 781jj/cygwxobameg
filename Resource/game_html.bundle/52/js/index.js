! function(e, t) {
    e("data/jigsawGame", function(e, t, n) {
        ...
        function r(e) {
            ...
            p.dom.find(".kapics").html("the "+(p.currentGame.pic + 1) + "/" + n.pic.length+" level"), u.set(f, p.currentGame), p.jigsawGame.setConfig(r)

            /*, only "+r.second+" secs this level*/
        ...
        p.init = function(e) {
            ...
            e.html(t), p.jigsawGame = s, p.jigsawGame.init(e), p.dom = e, p.jigsawGame.on("picOver", function(e) {
				...
				var str="Level: "+(p.currentGame.pic+1)+"</em>";
				if(!p.hasNextPic()){
					str="Congratulations! mission completed!";
				}
				showGameResult(str,true,p.hasNextPic());
            }), p.jigsawGame.on("timeout", function() {
				showGameResult("Game Over!",false,p.hasNextPic()); 
            })
        ...
        p.saveUserScore = function(e) {
            ...
            if (location.hash == "#rd") picka = "1,0";
            picka = picka.split(",");
            var e = {"type":"jigsaw","ka":0|picka[0],"pic":0|picka[1]};

            !e || 0 == e.ka && 0 == e.pic ? (p.dom.find(".play-info").show().find(".first-guide").html("exchange the pictures to finish the jigsaw").show(), e = {
                type: c[0].type,
                ka: 0,
                pic: 0
            }) : p.dom.find(".play-info").hide().find(".first-guide").html("come!"), r(e)
        ...
        p.showNextKa = function() {
			...
			(p.dom.find(".first-guide").html("Congratulations! task is completed"), p.dom.find(".play-info").show().find(".playing-over").show())
			if(p.hasNextPic()){
				//p.do
			}else{
			}
            ...
            new m({
                message: "Congratulations! Game Over",
                calcelTxt: "Close",
                sureTxt: "Replay",
                sure: function() {
                    p.currentGame = {
        ...
        l.showPage = function(e, t) {
            ...
            if (m.curPage == m.pageIndex[0]) navigator.notification.confirm("Exit?", function(e) {
