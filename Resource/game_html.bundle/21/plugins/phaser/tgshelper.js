...
Phaser.Plugin.TreSensaPlugin.prototype._positionWidget = function () {
	...
	var scaleObj = this.game.scale ? this.game.scale : this.game.stage.scale;
	var scale = Math.max(scaleObj.scaleFactorInversed.x,scaleObj.scaleFactorInversed.y);

	return {
		x: this.game.canvas.offsetLeft + this.widgetX*scale,
		y: this.game.canvas.offsetTop + this.widgetY*scale,
