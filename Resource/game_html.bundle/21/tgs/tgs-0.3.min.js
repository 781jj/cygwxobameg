/* TreSensa Game Services, TGS-SDK, Version 0.3.23 */
...
TGS.PartnerBridge = function() {
    ...
    this._mMoreGamesTarget = window.self === window.top ? "_self": "_blank",
    this._mSplashScreen = !1,
    this._mSplashTimer = 3,
    this._mSplashBackgroundColor = "#fff",
    this._mShowBookmarkPrompt = !1,
    this.freeIAP = !1,
    this.freeIAPTitle = "Woo-hoo!",
    this.freeIAPMessage = "You've just unlocked this item for free!",
    this.autoUnlockNonConsumables = !1,
    this.offerValueExchange = !1,
    this.valueExchangeMessage = "Woo-hoo! You just unlocked this item for free, thanks to the following ad from our sponsors:",
...
TGS.PartnerBridge.prototype = {
    ...
    leaderboardLabel: function(a) {
        return a ? "Submit Score": "Leaderboard"
    },
    ...
    share: function(a, b) {
        ...
        // } else if ("fb" == a) if ("string" != typeof window.location.host || -1 === window.location.host.indexOf("tresensa.com")) {
    ...
    initFacebook: function() {
        if (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "PartnerBridge.initFacebook called"), !this._mFacebookInitialized) {
            ...
            function(b) {
                TGS.Debug.Log(TGS.Debug.LOG_INFO, "the Facebook session status changed to " + b.status),
                a.onGetFBLoginStatus(b)
            }),
    ...
    //     "connected" === a.status ? (TGS.Debug.Log(TGS.Debug.LOG_INFO, "user is logged into Facebook and authorized the app"), FB.api("/me",
    //     function(a) {
    ...
    showSplashScreen: function(a) {
        if (!this._mSplashScreen) return void(a.closeCallback && a.closeCallback());
        var b = document.createElement("div");
        b.id = "tgs-splash",
        b.style.zIndex = 1e3,
        b.style.backgroundColor = this._mSplashBackgroundColor,
        b.style.top = 0,
        b.style.left = 0,
        ...
        e.src = ""; //"//sdk.tresensa.com/tgs/" + TGS._sPartnerID + "/splash.png",
        e.style.width = "100%",
        d.insertBefore(e, d.firstChild);
        var f = .5 * window.innerWidth - c / 2,
        g = .45 * window.innerHeight - c / 2;
        d.style.top = g.toString() + "px",
        d.style.left = f.toString() + "px",
        this._mSplashScreen = !1,
        setTimeout(this.closeSplashScreen.bind(this, b, a.closeCallback), 1e3 * this._mSplashTimer)
    },
    closeSplashScreen: function(a) {
        this._splashOpacity = 100,
        this._splashTimeoutID = setInterval(this.fadeSplashScreen.bind(this, a), 10)
    },
    fadeSplashScreen: function(a, b) {
        this._splashOpacity -= 2,
        a.style.opacity = this._splashOpacity / 100,
        a.style.filter = "alpha(opacity=" + this._splashOpacity + ")",
        this._splashOpacity <= 0 && (a.parentNode.removeChild(a), clearInterval(this._splashTimeoutID), b && b())
    },
    advertising_injectAd: function(a, b) {
        ...
        // var d = this.advertising_getAdServerURL(c, a.placement ? 0 : 170);
        ...
        // 3 === a.placement ? f = "<div width='" + a.width + e + "' height='" + a.height + e + "' id='tgs-ad-frame' class='tgs-ad-frame'><div class='value-exchange-message'>" + this.valueExchangeMessage + "</div><span>Advertisement</span>" + f + "</div>": a.placement && (f = "<div width='" + a.width + e + "' height='" + a.height + e + "' id='tgs-ad-frame' class='tgs-ad-frame'><span>Advertisement</span>" + f + "</div>"),
        // a.element.innerHTML = f,
    ...
    advertising_injectInterstitial: function(a) {
        ...
        //     r.addEventListener("touchend", l, !0)
        // },
        // l = function() {
        ...
        // p.id = "tgs_ad_overlay",
        // p.style.zIndex = 1e3,
        // p.style.backgroundColor = "rgba(" + Math.round(255 * e).toString() + "," + Math.round(255 * f).toString() + "," + Math.round(255 * g).toString() + "," + h.toString() + ")",
        // p.style.top = 0,
        // p.style.left = 0,
        // b != document.body && n && o ? (p.style.position = "absolute", p.style.width = n + "px", p.style.height = o + "px") : (TGS.Debug.Log(TGS.Debug.LOG_WARNING, "PartnerBridge.advertising_injectInterstitial: Could not determine width/height of parentDiv, going fullscreen with overlay"), p.style.position = "fixed", p.style.width = "100%", p.style.height = "100%"),
        // b.insertBefore(p, b.firstChild);
        // var q = new TGS.Advertisement(a || {},
        // a.placement);
        // q.closeCallback = l,
        // q._inject(p);
        // var r = document.createElement("img");
        // r.id = "tgs-close",
        // r.src = TGS._IMAGES_LOCATION + "close-button.png",
        ...
        // d > 0 ? q.showCloseTimeout = setTimeout(k, 1e3 * d) : k();
        ...
        // c && c.classList.add("tgs-blur")
    },
    advertising_getAdServerURL: function(a, b) {
        var c = TGS._sConfig.NATIVE_APP || TGS._sConfig.TIZEN_APP ? "http:": "";
        return c = [/*"//adserver.adtechus.com/adiframe/3.0/5421.1/"*/ "", a, "/0/", b, "/ADTECH;target=_blank", ";key=", TGS._sConfig.DST_ID, ";kvenv=", encodeURIComponent(TGS.BrowserDetect.platform), ";sub1=", encodeURIComponent(TGS._sGameID)].join("")
    },
...
TGS.Microtransactions.PurchaseProduct = function(a) {
    ...
    TGS._sPartnerBridge.logTransactionBeforePartnerRequest() ? TGS.Microtransactions._LogPurchaseRequest(k) : (TGS.Debug.Log(TGS.Debug.LOG_INFO, "requesting purchase from partner..."), TGS.Microtransactions._DoPurchaseRequest(k))
},
...
TGS.Microtransactions._DoPurchaseRequest = function(a) {
    ...
    if (b.freeIAP) {
        a.partnerTransactionID = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,
        function(a) {
            var b = 16 * Math.random() | 0,
            c = "x" == a ? b: 3 & b | 8;
            return c.toString(16)
        ...
        closeCallback: function() {
            ...
            TGS.Microtransactions._PurchaseOverlay.style.display = "block",
            TGS.Microtransactions._PartnerPurchaseSuccessful(a)
        }
...
function() {
    ...
    var a = {
        number: function(a, b) {
            var c = a.toString(),
            d = !1;
            return b.decimal && (d = c.substr( - b.decimal), c = c.substr(0, c.length - b.decimal)),
            c.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + (b.decimal ? "." + d: "")
        },
        time: function(a, b) {
            for (var c = a / 1e3,
            d = 0; c >= 60;) d += 1,
            ...
            g = d;
            return b.decimal >= 1 && (g += ":" + (e > 9 ? e: "0" + e)),
            b.decimal >= 2 && (g += ":" + (f > 9 ? f: "0" + f)),
            g
        }
    },
    b = function(b, c) {
        ...
        e = d.scoreFormat || {};
        e.type = (e.type || "number").toLowerCase(),
        e.decimal = e.decimal || ("time" == e.type ? 2 : 0),
        e.prefix = e.prefix || "",
        e.suffix = e.suffix || "";
    ...
    //         if ( - 1 === window.location.host.indexOf("tresensa.com")) return (f && a.score < TGS.localStorage.getItem(g) || !f && a.score > TGS.localStorage.getItem(g)) && TGS.localStorage.setItem(g, a.score),
    //         TGS.Debug.Log(TGS.Debug.LOG_WARNING, "user is not logged in/authorized on Facebook - saving score"),
    //         void(a.onSuccess && a.onSuccess.call(this, a.score, a.leaderboardID));
    //         if (!TGS._sPartnerBridge.loggedIntoFacebook()) return b ? void TGS._sPartnerBridge.loginToFacebook(TGS._sPartnerBridge.promptForFacebookAuthorization.bind(TGS._sPartnerBridge), TGS.Leaderboard._SubmitScoreSLB.bind(this, a)) : ((f && a.score < TGS.localStorage.getItem(g) || !f && a.score > TGS.localStorage.getItem(g)) && TGS.localStorage.setItem(TGS._sLSKeys[g], a.score), void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "user is not logged in/authorized on Facebook - saving score"));
    //         c = TGS._sPartnerBridge._mFacebookUserID,
    //         d = "A0003"
    //     }
    //     var h = !0,
    //     i = "";
    //     if ("object" != typeof a ? (h = !1, i += "params ") : "number" != typeof a.score && (h = !1, i += "score "), !h) return TGS.Debug.Log(TGS.Debug.LOG_ERROR, "Invalid values sent to TGS.Leaderboard.SubmitScore for the following params: " + i),
    ...
    //     n.type = "add",
    //     n.params = a,
    //     n.onSuccess = a.onSuccess,
    //     n.onFailure = j,
    //     TGS.Debug.Log(TGS.Debug.LOG_INFO, "submitting new score of " + a.score + " to server..."),
    //     TGS.SendMessage("leaderboard/score", "add", m, TGS.Leaderboard._SuccessCallback.bind(this, n), TGS.Leaderboard._ErrorCallback.bind(this, n))
    },
    TGS.Leaderboard._ShowSLB = function(a) {
        // var d, e = (document.getElementsByTagName("base"), document.createElement("a"));
        // e.href = "",
        // d = e.href;
        // var f = a.score,
        ...
        // };
        // if (TGS.localStorage.removeItem(TGS._sLSKeys.TGS_pending_score + a.leaderboardID), TGS._sPartnerBridge._mUseUserInfoInSLB) j.userInfo = {
        //     id: TGS._sUserID,
        ...
        // };
        // else if ( - 1 === window.location.host.indexOf("tresensa.com")) j.partner = "A0003";
        // else {
        ...
        // }
        // var k;
        // //k = TGS._sConfig.PROD_ENV ? "//leaderboard.tresensa.com/?inGame=true&gameInfo=" + encodeURIComponent(JSON.stringify(j)) : "//qa-leaderboard.tresensa.com/?inGame=true&gameInfo=" + encodeURIComponent(JSON.stringify(j));
        ...
        //     "leaderboard:done" == b ? q() : "leaderboard:challenge" == b && r()
        // };
        // window.addEventListener("message", s, !1),
        // TGS.Analytics.logScreen("leaderboard")
    },
...
function() {
    ...
    TGS.Advertisement.prototype = {
        ...
        //     if (!a.origin.match("^https?://adserver.adtechus.com$")) return void TGS.Debug.Log(TGS.Debug.LOG_WARNING, "Received window message: " + a.data + " from unexpected origin: " + a.origin);
        //     var b = JSON.parse(a.data);
        //     TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "Received window message: " + b),
        //     "close" == b ? this.close() : "fullscreen" == b ? this._fullscreen() : "disable-close" == b ? this._disableClose() : b && "resize" == b.action && this._resize(b.width, b.height)
        },
        ...
        _reposition: function(a) {
            ...
            //     var i = this.height,
            //     j = this.width;
            //     this.placement && (i += 52, j += 52, g -= 26, h -= 26),
            //     this.scale = 1,
            //     this.placement ? this.scale = f > e ? .75 * e / i: .75 * f / j: TGS.Utils.isTGEGame() && (this.scale = TGE.Game.GetInstance()._mDivResized || 1),
    ...
    TGS.Advertisement.DisplayAd = function(a) {
        ...
        return ("undefined" == typeof navigator.onLine || navigator.onLine) && (b._inject(a.parentDiv), TGS.Analytics._logAdEvent("display", a.level)),
        b
    },
    ...
    TGS.Advertisement._DisplayPreloaderAd = function(a) {
        a = a || {};
        var b = TGS.AdapterDefinitions[TGS._sPartnerID];
        a = "undefined" == typeof a ? {}: a,
        a.placement = 2,
        b && b.preloader && ("undefined" == typeof navigator.onLine || navigator.onLine) ? (TGS._sPartnerBridge.advertising_injectInterstitial(a || {}), TGS.Analytics._logAdEvent("pre-loader", a.level)) : a.closeCallback && a.closeCallback()
    },
    TGS.Advertisement.DisplayValueExchangeAd = function(a) {
        a = a || {},
        a.placement = 3,
        "undefined" == typeof navigator.onLine || navigator.onLine ? (TGS._sPartnerBridge.advertising_injectInterstitial(a || {}), TGS.Analytics._logAdEvent("valueexchange", a.level)) : a.closeCallback && a.closeCallback()
    }
} (),
function() {
    var a = function(a) {
    ...
    c = {
        ...
        achievement: b,
        share: ["twitter", "facebook", "fb", "kik", "system_share"],
        clickthru: b,
        custom: b,
        error: b,
        moregames: b,
    ...
    TGS.Analytics = {
        _init: function(a) {
            ...
            if (!g) {
                for (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init called with config: " + JSON.stringify(a)), f = a, b = TGS._sPartnerBridge.analytics_getProviders() || [], TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init got " + b.length + " providers from partner bridge"), c = 0; c < b.length; c++) try {
                    ...
                    TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init initializing provider: " + j.name),
                    j.init(f[j.configName] || {}) !== !1 ? e.push(j) : TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "TGS.Analytics.init " + j.name + " returned false from init, not using")
                } catch(k) {
        ...
        logShareEvent: function(a, b, c) {
            TGS.Debug.Log(TGS.Debug.LOG_WARNING, "TGS.Analytics.logShareEvent is deprecated and should not be called directly"),
            i("share", a, b, c)
        },
        logLevelEvent: function(a, b, c, d) {
            d = d || {},
            d.level = b;
            var e; ("complete" == a || "fail" == a && null !== c) && (e = c),
            i("level", a, e, d)
        },
...
function() {
    ...
    function(b) {
        ...
        j = h.Hasher,
        h = g.algo,
        k = [], l = 0; 64 > l; l++) k[l] = 4294967296 * b.abs(b.sin(l + 1)) | 0;
        h = h.MD5 = j.extend({
            _doReset: function() {
            ...
            _doFinalize: function() {
                ...
                for (c[(e + 64 >>> 9 << 4) + 15] = 16711935 & (f << 8 | f >>> 24) | 4278255360 & (f << 24 | f >>> 8), c[(e + 64 >>> 9 << 4) + 14] = 16711935 & (d << 8 | d >>> 24) | 4278255360 & (d << 24 | d >>> 8), a.sigBytes = 4 * (c.length + 1), this._process(), a = this._hash, c = a.words, d = 0; 4 > d; d++) e = c[d],
    ...
    TGS.Analytics.GameAnalyticsProvider = function() {},
    TGS.Analytics.GameAnalyticsProvider.prototype = {
        name: "GameAnalytics",
        configName: "GameAnalytics",
        init: function(a) {
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "GameAnalyticsProvider.init called: " + JSON.stringify(a)),
            this._mConfig = a;
            var b = [TGS._sGameVersion, "web"];
            TGS._sConfig.PROD_ENV || b.push("sandbox"),
            this._mBuild = b.join(" "),
            this._mSessionId = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,
            function(a) {
                var b = 16 * Math.random() | 0,
                c = "x" == a ? b: 3 & b | 8;
                return c.toString(16)
        ...
        _apiCall: function(b, c) {
            // var d = this._mConfig.GAME_KEY,
            // e = this._mConfig.SECRET_KEY;
            // c = c || "design";
            // var f = ("https:" == location.protocol ? "https": "http") + "://api-eu.gameanalytics.com/1/" + d + "/" + c,
            // g = JSON.stringify(b),
            // h = a.MD5(g + e),
            // i = a.enc.Hex.stringify(h);
            // TGS.CORSRequest(f, g, !1, !0,
            // function() {},
            // function() {},
            // {
            //     Authorization: i
        ...
        trackScreen: function(a) {
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "GameAnalyticsProvider.trackPageView called: " + JSON.stringify(arguments)),
            this.trackEvent("screen", a)
        },
...
function() {
    ...
    TGS.Analytics.GoogleAnalyticsProvider.prototype = {
        ...
        init: function(a) {
            ...
            this._mConfig = a,
            this._mConfig.STUDIO_ONLY || (_gaTreSensa("create", TGS._sConfig.PROD_ENV ? "UA-29301358-4": "UA-53157750-2", "auto", {
                name: "TGSTracker"
            ...
            }), TGS.MicrotransactionsSupported() && _gaTreSensa("TGSTracker.require", "ecommerce", "ecommerce.js"), _gaTreSensa("TGSTracker.send", "pageview")),
            ...
            !this._mConfig.STUDIO_ONLY && TGS._sConfig.PROD_ENV && TGS._sPartnerBridge._GoogleAnalyticsId && (_gaTreSensa("create", TGS._sPartnerBridge._GoogleAnalyticsId, "auto", {
        ...
        trackEvent: function(a, b, c, d) {
            ...
            f = {};
            e = e[this.configName] || {},
            e.nonInteraction && (f.nonInteraction = !0),
            ("level" == a || "ad" == a) && d.level > 0 && (b += " - level " + d.level),
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "GoogleAnalyticsProvider.trackEvent called: " + JSON.stringify(arguments)),
            this._mConfig.STUDIO_ONLY || "ads" == a || "achievement" == a || "custom" == a || "game" == a && "resume" == b || _gaTreSensa("TGSTracker.send", "event", a, b, this._mConfig.LABEL || TGS._sGameID, c, f),
            "moregames" == a && (b = "URL launched"),
            this._studioTrackerEnabled && !e._tresensaOnly && _gaTreSensa("TGSStudioTracker.send", "event", a, b, this._mConfig.LABEL || TGS._sGameID, c, f),
            this._partnerTrackerEnabled && !e._tresensaOnly && "ads" != a && "achievement" != a && "custom" != a && _gaTreSensa("TGSPartnerTracker.send", "event", a, b, this._mConfig.LABEL || TGS._sGameID, c, f)
        },
        trackScreen: function(a, b) {
            ...
            this._mConfig.STUDIO_ONLY || "leaderboard" != a || _gaTreSensa("TGSTracker.send", "pageview", TGS._sConfig.PATH + a),
            this._studioTrackerEnabled && !c._tresensaOnly && _gaTreSensa("TGSStudioTracker.send", "pageview", TGS._sConfig.PATH + a),
            this._partnerTrackerEnabled && !c._tresensaOnly && "leaderboard" == a && _gaTreSensa("TGSPartnerTracker.send", "pageview", TGS._sConfig.PATH + a)
        },
        trackPurchase: function(a) {
...
function() { !
    function(a, b) {
        if (!b.__SV) {
            ...
            c.async = !0,
            c.src = ""; //"//cdn.mxpnl.com/libs/mixpanel-2.2.min.js",
            d = a.getElementsByTagName("script")[0],
            d.parentNode.insertBefore(c, d)
        }
    } (document, window.mixpanel || []),
    TGS.Analytics.MixPanelProvider = function() {},
    TGS.Analytics.MixPanelProvider.prototype = {
        name: "MixPanel",
        configName: "MixPanel",
        init: function(a) {
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "MixPanelProvider.init called: " + JSON.stringify(a)),
            this._mConfig = a,
            mixpanel.init(this._mConfig.TOKEN),
        ...
        updateUserData: function(a) {
            ...
            a.age_range && (b.age_range = a.age_range.min + " - " + a.age_range.max),
            mixpanel.people.set(b)
        },
        trackEvent: function(a, b, c, d) {
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "MixPanelProvider.trackEvent called: " + JSON.stringify(arguments)),
            d = d || {};
            var e, f = {
                category: a,
                value: c
            };
            e = "game" == a || "level" == a ? a + " " + b: "custom" == a ? b: a,
            "achievement" == a ? f["achievement name"] = b: "error" == a ? f["error message"] = b: "share" == a ? f["share service"] = b: "challenge" == a ? f["challenge event"] = b: "moregames" == a || "clickthru" == a ? f["clickthru url"] = b: "ads" == a && (f.placement = b);
            var g = d.settings;
            delete d.settings,
            mixpanel.track(e, merge(f, d)),
            d.settings = g
        },
        trackScreen: function(a, b) {
            b = b || {},
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "MixPanelProvider.trackPageView called: " + JSON.stringify(arguments)),
            delete b.settings,
...
function() {
    ...
    TGS.Analytics.NativeGoogleAnalyticsProvider.prototype = {
        ...
        trackPurchase: function(a) {
            ...
            this._gaPlugin.trackPurchase(this._logResult, this._logError, a.transactionID, TGS.Microtransactions.PaymentProviderName(), a.item.id, a.item.title, TGS.Microtransactions.PriceAsUSD(a.item.price), 1, TGS.Microtransactions.GetIAPProduct(a.item.id).consumable ? "consumable": "non-consumable")
        },
...
function() {
    TGS.Analytics.NativeMixPanelProvider = function() {},
    TGS.Analytics.NativeMixPanelProvider.prototype = {
        name: "MixPanel",
        configName: "MixPanel",
        init: function(a) {
            return TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "NativeMixPanelProvider.init called: " + JSON.stringify(a)),
            this._mConfig = a,
            window.plugins && window.plugins.mixpanel ? (this._mpPlugin = window.plugins.mixpanel, this._mpPlugin.sharedInstanceWithToken(this._mConfig.TOKEN), void this._mpPlugin.identify(TGS._sUserID)) : (TGS.Debug.Log(TGS.Debug.LOG_ERROR, "PhoneGap MixPanel plugin was not found, disabling analytics"), !1)
        },
        updateUserData: function(a) {
            mixpanel.identify(TGS._sUserID);
            var b = {
        ...
        trackEvent: function(a, b, c, d) {
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "NativeMixPanelProvider.trackEvent called: " + JSON.stringify(arguments)),
            d = d || {};
            var e, f = {
                category: a,
                value: c,
            ...
            var g = d.settings;
            delete d.settings,
            this._mpPlugin.track(e, merge(f, d)),
            d.settings = g
        },
        trackScreen: function(a, b) {
            b = b || {},
            TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "NativeMixPanelProvider.trackPageView called: " + JSON.stringify(arguments)),
            delete b.settings,
            this._mpPlugin.track("screen", merge({
                category: "screen",
                "screen name": a,
                "Acquisition Source": TGS._sPartnerID,
                version: TGS._sGameVersion,
...
function() {
    TGS.Social = {
        ...
        Share: function(a, b) {
            b = b || {};
            var c;
            c = b.url ? b.url: TGS.BrowserDetect.usingPhoneGap ? TGS._sConfig.HOST: window.self != window.top ? document.referrer: window.location.href,
...
function() {
    ...
    TGS.Widget = function(c) {
        ...
        this.closeCallback = c.closeCallback,
        ...
        this.updatePlacement = function() {
            var a = 1,
            b = d();
            "number" == typeof b ? a = b: (a = b.scale ? b.scale: a, e.pos.top = "number" == typeof b.y ? b.y: e.pos.top, e.pos.left = "number" == typeof b.x ? b.x: e.pos.left),
            e.element.style.top = e.pos.top + "px",
            e.element.style.left = e.pos.left + "px",
            e.element.style.webkitTransform = "scale(" + a + ")",
            e.element.style.MozTransform = "scale(" + a + ")",
            e.element.style.msTransform = "scale(" + a + ")",
            e.element.style.OTransform = "scale(" + a + ")",
            e.element.style.transform = "scale(" + a + ")"
        },
        c.placementFunc ? d = c.placementFunc: c.scalingFunc ? d = c.scalingFunc: TGS.Utils.isTGEGame() && TGE.Game.GetInstance()._mDivResized && (d = b),
        ...
        l = j * k / 100;
        c.widgetClass && this.element.classList.add(c.widgetClass),
        i && this.element.classList.add("horizontal");
        var m = 1;
        if (c.disableAd && this.size.width >= 4 * h + 3 * l ? (this.element.classList.add("four-column"), m = 4) : (!i || c.disableAd) && this.size.width >= h + l + h && (this.element.classList.add("two-column"), m = 2), c.disableAd || (this.ad = new TGS.Advertisement({}), this.ad.widgetOpts = this.opts, this.ad._inject(this.element), this.ad.element && (this.ad.element.style.position = "static"), TGS.Analytics._logAdEvent("display")), this.buttonContainer = document.createElement("div"), this.buttonContainer.classList.add("btn-container"), i && (this.buttonContainer.style.width = j + "px"), c.showLogin) {
        ...
        if (!c.disableShare) for (var q = TGS.Social.GetShareServices(2), r = 0; r < q.length; r++) {
            var s = q[r],
            t = {
        ...
        if (!c.disableMoreGames && TGS.MoreGamesURL() && this.buttonContainer.appendChild(a("More Games", TGS.ShowMoreGames, "moregames")), !c.disableLeaderboard && TGS.Leaderboard.isSupported() && Object.keys(TGS.Leaderboard.GetLeaderboardDescriptors()).length > 0) {
            var u = TGS.Leaderboard.Show.bind(this, {
                leaderboardID: c.leaderboardID || 0,
        ...
        var v = function() {
            if (m > 1) {
                ...
                for (var a = e.buttonContainer.childNodes,
                b = a.length - 1; b >= 0; b--) {
                    var c = b == a.length - 1;
                    a[b].style.marginBottom = k + "%",
                    2 == m && a.length % 2 != 0 && c ? a[b].style.width = "100%": ((4 == m && !c || b % 2 === 0) && (a[b].style.marginRight = k + "%"), a[b].style.width = (100 - (m - 1) * k) / m + "%")
                }
            }
        };
        v(),
...
function() {
    ...
    TGS.Language = {
        PreferredLanguage: function() {
            ...
            return b || TGS._sPartnerBridge.preferredLanguage() || a
        }
    }
} (),
function() {
    TGS.Adapters = {};
    var a = "";//"stg-tgs.tresensa.com";
    "undefined" != typeof window.GameConfig && GameConfig.TGS && GameConfig.TGS.HOST && (a = GameConfig.TGS.HOST);
    var b = a + "/server/";
    b += "0.3.9/",
    TGS._SERVER_LOCATION = ("https:" === document.location.protocol ? "https:": "http:") + "//" + b,
    TGS._IMAGES_LOCATION = ("https:" === document.location.protocol ? "https:": "http:") + "./tgs/images/",
    TGS._sOn = !1,
    ...
    TGS.GameLaunched = function() {
        if (TGS._sPartnerBridge) {
            if (TGS._sPartnerBridge.autoUnlockNonConsumables) for (var a = TGS.Microtransactions.GetIAPProducts(), b = 0; b < a.length; b++) {
                var c = a[b];
    ...
    TGS.CORSRequest = function(a, b, c, d, e, f, g) {
        var h = null,
        i = "POST",
        j = null,
        k = !1;
        ...
        try {
            if (window.XMLHttpRequest) {
                // 不知道有没有用的一段 ！！！！
                // if (h = new XMLHttpRequest, "withCredentials" in h) h.open(i, a, !0),
                // l(h, g),
                // h.send(b);
                // else if ("undefined" != typeof XDomainRequest) {
                //     k = !0,
                //     h = new XDomainRequest;
                //     var m = Math.floor(99999999999 * Math.random()),
                //     n = -1 === a.indexOf("?") ? "?": "&";
                //     a = a + n + b + "&cb=" + m,
                //     h.open(i, a),
                //     l(h, g),
            ...
            f && f("server failed to respond", o)
        }
        try {
            k ? (h.timeout = 1e4, h.onerror = function() {},
            h.ontimeout = function() {},
            h.onprogress = function() {},
            h.onload = function() {
                if (!d) return void e(h.responseText);
                try {
                    j = JSON.parse(h.responseText)
                } catch(a) {
                    TGS.Debug.Log(TGS.Debug.LOG_WARNING, "server response could not be parsed: " + h.responseText),
                    j = null
                }
                if (j) {
                    if (c && 0 !== j.code && f) {
                        var b = "the TGS server returned error code " + j.code;
                        return "string" == typeof j.message && (b += " " + j.message),
                ...
                if (4 == h.readyState) if (200 != h.status) f && (TGS.Debug.Log(TGS.Debug.LOG_WARNING, "the page returned error code " + h.status + " message: " + h.statusText), f("the page returned error code " + h.status, h));
                else {
                    if (!d) return void e(h.responseText);
                    try {
                        j = JSON.parse(h.responseText)
    ...
    TGS.Init = function(a, b) {
        ...
        TGS._sLSKeys.TGS_pending_score = "TGS_pending_score" + d,
    ...
    TGS.PromptToBookmark = function() {
        d && addToHomescreen.isCompatible && TGS._sPartnerBridge._mShowBookmarkPrompt && d.show()
    },
    ...
    TGS._LoadAdapterLibraries = function() {
        ...
        b = [],
        c = TGL.SDKBaseURL + "tgs/", // window.TGL ? TGL.SDKBaseURL + "tgs/": "//sdk.tresensa.com/tgs/",
    ...
    TGS.addEventListener = function(a, b) {
        c[a] = c[a] || [],
        c[a].push(b)
    },
    TGS.removeEventListener = function(a, b) {
        if (b) {
            if (c[a]) {
                var d = c[a].indexOf(b); - 1 !== d && c[a].splice(d, 1)
            }
        } else c[a] = []
    },
    TGS.dispatchEvent = function(a, b) {
        if (c[a]) for (var d = 0; d < c[a].length; d++) {
            var e = c[a][d];
            try {
                e.call(null, b)
            } catch(f) {
                TGS.Debug.Log(TGS.Debug.LOG_ERROR, "Error while dispatching event [" + a + "]: " + f)
            }
        }
    },
    TGS.MakeTGSReady = function() {
        if (TGS.Debug.Log(TGS.Debug.LOG_VERBOSE, "MakeTGSReady called"), TGS.IsReady()) {
            if (TGS._sPartnerBridge._mShowBookmarkPrompt && (d = addToHomescreen({
                autostart: !1,
                ...
                startDelay: 0,
                onAdd: function() {
                    TGS.Debug.Log(TGS.Debug.LOG_INFO, "Add to homescreen detected. Triggering 'bookmarked' event"),
                    TGS.Analytics.logGameEvent("bookmarked"),
                    setTimeout(function() {
                        TGS.dispatchEvent("bookmarked")
                    },
                    0)
                }
            }), TGS.PromptToBookmark()), window.TGE) {
                var a = TGE.Game.GetInstance();
    ...
    TGS._DisplaySplashScreen = function(a) {
        a = a || {},
        TGS._sPartnerBridge.showSplashScreen(a)
    },
    ...
    TGS.OpenURL = function(a, b, c, d) {
        ...
        d || TGS.Analytics._logClickThruEvent(a)
    },
...
TGS.Adapters.Q0000 = function() {
    ...
    TGS.DataStore._sSaveToLocalStorage = !0,
    TGS.DataStore._sSaveToTGSServer = !1,
    this._GoogleAnalyticsId = "UA-53157750-1",
    this._mShowBookmarkPrompt = !0
},
