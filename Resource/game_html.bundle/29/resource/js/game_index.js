...
if(!isWeiXin()){
	location.href="http://www.duopao.com";
}
...
function dp_submitScore(vLevel,vScore){
    crossAjax({
        ...
        data:{
            ...
            game_level:vLevel,
            game_score:vScore,
            uid: QueryString.getParameter('uid')
        },
        type:'post',
        dataType:'json',
...
//                   }
                ...
                if(res.bestscore==1){//最好记录
                    if(confirm("恭喜您获得了最好的成绩，是否要秀给你的小伙伴们？")){
                        location.href=res.url;
                    }
                }
            }else if(res.flag=='f1'){
                if(loginCounter==2||confirm('您尚未登录，游戏积分无法上传到排行榜，是否登录？')){
...
*/
function dp_getGameResulte(level, score, callback,freflag,levflag){
    crossAjax({
        url: rootIndex+'games/fups',
        data:{
            game_code:QueryString.getParameter('game_code'),
            game_level:level,
            game_score:score,
            freflag:freflag,
            levflag:levflag,
            cc:QueryString.getParameter('cc'),
            uid: QueryString.getParameter('uid') || -1
        },
        type:'post',
        dataType:'json',
        success:function(res){
            if(res.flag=='f0'){
                var resulte = new Object();
                resulte.concern = res.concern; 
                resulte.rank = res.ulevel; 
                resulte.url = res.url; 
                resulte.othersInfo = [];
                var _others = resulte.othersInfo, _othersData = res.retlist || [];
                for(var i = 0; i < _othersData.length; i++){
                    _others[i] = new Object();
                    _others[i].name = _othersData[i].user_nickname;
					_others[i].avatar = new Image();
                    _others[i].avatar.src = _othersData[i].user_avatar;
                    _others[i].score = _othersData[i].user_score;
                }
                if(callback){
                    callback(resulte);
                }
                ...
                if(loginCounter==2||confirm('您尚未登录，游戏积分无法上传到排行榜，是否登录？')){
                    location.href='http://uc.duopao.com/uc/wxrect?url='+encodeURIComponent(location.href);
                }else{
                    loginCounter++;
                }
                return null;
    ...
    crossAjax({
        ...
        success:function(result){
            ...
            if(data.flag=='f0'||data.flag=='f1'){
                var tem;
                if(data.flag!='f0'){
                    location.href='http://www.duopao.com/games/more';
                }else{
                    location.href=rootIndex+'games/toplist?game_code='+game_code;
                    return;
                    if(data.ulevel==-1){
                ...
                tem += '</div>';
                tem += '<a ontouchstart="" class="w_button w_button_small fl" id="gobackgame" href="www.baidu.com" style="width: 50px; margin-top: 10px;">继续游戏</a>';
                tem += '<a ontouchstart="" class="w_button w_button_small fr" href="'+rootIndex+'games/index" style="width: 50px; margin-top: 10px;">更多游戏</a>';
                tem += '<a ontouchstart="" class="w_button w_button_small fr" href="'+rootIndex+'games/toplist?game_code='+game_code+'" style="width: 50px; margin-top: 10px;margin-right:17px;">更多排名</a>';
                win=new DWindow({
                    width:300,
                    title:'排行榜',
                    content:tem
	...
	function _createTopMenu () {
		...
		if (!_getTopMenu) {
			...
			_getTopMenu.innerHTML = [
				'<span style="position:relative;display:inline-block;width:20%;text-align:left;"><a id="directionMenuBtn_0" href="javascript:void(0);" class="menuBtn2 radius" style="top:-11px;margin-left:5px;"><span id="directionMenuBtn_1" class="menuBtn2-icon"></span><span id="directionMenuBtn_2" class="menuBtn2-desc">duopao.com</span></a></span>',
				'<span style="position:relative;display:inline-block;width:60%;text-align:center;"><a href="http://www.duopao.com"><img id="directionMenuBtnDuoPao" src="../resource/img/iconmap_topheader.png" /></a></span>',
