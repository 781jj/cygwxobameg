...
(function() {
	...
	e.action = {}, e.action.Role = function(e, t, r, i, s, o, u, a) {
		...
		for (var a = 0, f; f = this.links[a]; a++) f.setSprite(t, n, r);
		return i = trans = o = u = null, this
	}, e.action.Role.prototype.addLinks = function(t) {
		this.links = t || [];
		for (var n = 0, r; r = this.links[n]; n++) r.setSprite(this.getSprite().trans == e.trans.TRANS_NONE ? this.current : -this.current).setStep(this.step);
		return this
		...
		if (r && r.args)
			if (i = this.frames[r.args[0]]) {
				...
				var c = 0,
					...
					p = this.aabbAR;
				while (c++ < 2) {
					for (var d = 0, v; v = h[d]; d++) v[0] > a && (a = v[0]), v[0] < u && (u = v[0]), v[1] > l && (l = v[1]), v[1] < f && (f = v[1]);
		...
		if (n) {
			...
			} else this.svx != null && this.svy != null && (this._skipMoveDs || (this.setSprite(this._stopedAction || this._stopDs[this.dsIndex = t(this.svx, this.svy)]), this._stopedAction = null), this.onend && this.onend(this), this._skipMoveDs = !1, this.svx = null, this.svy = null); if (this._zooms.length > 0) {
			...
			var a;
			for (var f = 0, l; l = this.links[f]; f++) l.x = this.x + (l.dx || 0), l.y = this.y + (l.dy || 0), l.getSprite().setFrame(e.current), l.updateFrameParam();
			a = null
		}
		...
		this.step = e || 0;
		for (var t = 0, n = this.sprites.length; t < n; t++) this.sprites[t].setStep(e);
		for (var r = 0, i; i = this.links[r]; r++) i.setStep(this.step);
		return this
		...
		for (var t = 0, n = this.sprites.length; t < n; t++) this.sprites[t].setLoop(e);
		return this
...
}), define("index", ["lib/link", "lib/action", "host", "shine"], function(e, t, n, r) {
	return {
		init: function() {
			var t = Date.now(),
				i, s, o = 640,
			...
			e.run(function() {
				...
				e.canvas.fillStyle("#FFF").fillScreen().drawImage("logo", i - 480 >> 1, u).drawImage("btns1", 0, 99, 480, 7, i - 480 >> 1, u + 140, 480, 7).drawImage("btns1", 0, 99, 480, 7, i - 480 >> 1, u + 555, 480, 7), e.buttonLayout.released("difficulty1") ? (l.moduleName = "Easy", c(2)) : e.buttonLayout.released("difficulty2") ? (l.moduleName = "ormal", c(3)) : e.buttonLayout.released("difficulty3") ? (l.moduleName = "Hard", c(4)) : e.buttonLayout.released("difficulty4") ? (l.moduleName = "Hardest", c(5)) : e.buttonLayout.released("rank") && dp_Ranking()
			}).zone(function() {

				e.canvas.fillStyle("#F00").fillScreen().fillStyle("#FFF").drawString(l.moduleName, 0, u + 110, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑").drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 140, 480, 7).drawImage("btns1", 0, 106, 480, 7, i - 480 >> 1, u + 555, 480, 7).fillStyle("#000").drawString(a(l.time, "S"), 0, u + 340, e.graphics.VCENTER, !1, null, null, "60px 微软雅黑").fillStyle("#000").drawString("Best:" + a(l.bestTime, ""), 0, u + 400, e.graphics.VCENTER, !1, null, null, "30px 微软雅黑"), l.time > l.bestTime && e.canvas.fillStyle("#FF0").drawString("New record", 0, u + 240, e.graphics.VCENTER, !1, null, null, "50px 微软雅黑"), e.buttonLayout.released("return") ? f() : e.buttonLayout.released("restart") && c(l.module)
					...
					Scene: e.extend(function(t) {
						...
						action: function(t, n, r, i, s) {
							...
							if (!this.hostDied) {
								...
								for (var f = this.boxes.length - 1, l; l = this.boxes[f]; f--) l.x += s, l.y = this.baseY - 5 - l.height, this.host.collisionInput(l.x, l.y, l.width, l.height, "bR") && (this.hostDied = !0, this.host.clearPath()), (l.x <= -l.width || l.x >= this.width) && this.boxes.splice(f, 1);
								o = null
							}
	...
	e.init(window.innerWidth, window.innerHeight).pushImage([{
		id: "logo",
		src: "noonedie_files/logo.jpg"
	}, {
		id: "btns1",
		src: "noonedie_files/btns1.png"
	}], function(e, t, n) {}).initAudio([{
		id: "1",
		src: "noonedie_files/1.mp3",
		preload: !0
