...
$(function() {
    ...
    c = {
        ...
        initEvent: function() {
            ...
            a.dialog.on(b, "#d-clear",
            function() {
                c.curGrid.data("val", "").data("mark", "").html(""),
                c.checkErr("empty")
            }),
            a.dialog.on(b, "#d-mark",
            function(b) {
                a.dialog.toggleClass("mark"),
                b.stopPropagation()
            }),
        ...
        showDiaolog: function() {
            ...
            if (this.closeAll(), a.grid.removeClass("active"), c.curGrid.addClass("active"), c.data.gridW && c.data.gridH || (c.data.gridW = c.curGrid.width(), c.data.gridH = c.curGrid.height(), c.data.chessboardW = $("#chessboard").width(), c.data.chessboardH = $("#chessboard").height()), b = Math.min(e.left - (150 - c.data.gridW) / 2, c.data.chessboardW - 150), b = Math.max(0, b), d = Math.min(e.top - (200 - c.data.gridH) / 2, c.data.chessboardH - 200), d = Math.max(0, d), a.dialog.find(".num").removeClass("on"), f) {
                ...
                for (var g = 0,
                h = f.length; h > g; g++) a.dialog.find(".num[data-val=" + f[g] + "]").addClass("on")
        ...
        draw: function() {
            ...
            a.chessboard.empty().html(b),
            a.grid = $("#grids span")
        },
        check: function() {
            c.checkErr(),
            c.data.tm.join("") == c.data.answer && (c.stopTimer(), localStorage.setItem("sudoku_lv" + c.data.lv, c.data.p), a.win.find(".lv").html(c.data.lv + 1), a.win.find(".time").html(a.time.html()), a.win.show(), _czc.push(["_trackEvent", "score", c.data.lv + 1]))
        },
        checkErr: function() {
            for (var b, d, e = parseInt(c.curGrid.data("idx")), f = Math.floor(e / 9), g = e % 9, h = 0, i = Math.floor(f / 3), j = Math.floor(g / 3), k = !1; 9 > h; h++) b = 9 * h + g,
            d = a.grid.eq(b),
            d.data("idx") != c.curGrid.data("idx") && (d.data("val") && d.data("val") == c.curGrid.data("val") ? (d.addClass("err"), k = !0) : d.removeClass("err")),
            b = h + 9 * f,
            d = a.grid.eq(b),
            d.data("idx") != c.curGrid.data("idx") && (d.data("val") && d.data("val") == c.curGrid.data("val") ? (d.addClass("err"), k = !0) : d.removeClass("err")),
    ...
    d = {
        data: {},
        init: function() {
            for (var a, b, c, d = $("#difficulty span"), e = 0; 5 > e; e++) a = parseInt(localStorage.getItem("sudoku_lv" + e) || 0),
        ...
        initEvent: function() {
            ...
            a.confirm.on(b, ".js-close",
            function() {
                c.closeConfirm()
            }),
            a.confirm.on(b, ".inner",
            function(a) {
                a.stopPropagation()
            }),
            a.restart.on(b,
