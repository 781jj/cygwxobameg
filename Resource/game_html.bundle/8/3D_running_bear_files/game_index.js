...
if (_getvp && _getvp.content.indexOf('width=960') >= 0) {
    var _ua = navigator.userAgent.toLowerCase(),
        _setW = window.innerWidth > window.innerHeight ? window.innerWidth : window.innerHeight;
...
function dp_submitScore(vLevel, vScore) {
    ...
    //                     str = "恭喜您击败了" + res.user_nickname + "，是否要秀给你的小伙伴们？";
    //                 } else {
    //                     str = "恭喜您获得了最好的成绩，是否要秀给你的小伙伴们？";
    //                 }
    //                 if (confirm(str)) {
...
window.addEventListener('load', function(e) {
    ...
    function _createTopMenu() {
        var _getCallMenuBtn = document.getElementById('callMenuBtn'),
            _getTopMenu = document.getElementById('topMenu');
        if (!_getCallMenuBtn) {
    ...
    _init();
    var _lastDate = Date.now(),
        _isPause = false,
        _playTimer, _rafRun, _state = 0,
        _bodyDesc = document.body.id;
    if (!_playTimer) {
