...
(function() {
    ...
    navigator.vibrate = function() {
        return navigator.vibrate || navigator.mozVibrate || navigator.webkitVibrate || navigator.oVibrate || navigator.msVibrate || (navigator.notification ?
        function(l) {
            ...
            function() {
                ...
                return {
                    ...
                    applyOne: function() {
                        ...
                        setTimeout(function() {
                            DOM.fadeOut(DOM.notification.block, 500,
                            function() {
    ...
    var cycleManager = {
        init: function(cycle, fpsMin) {
            ...
            if (window.kik && kik.browser && kik.browser.on) {
                kik.browser.on("background",
                function() {
                    cycleManager.stop()
                });
                kik.browser.on("foreground",
                function() {
                    cycleManager.resume()
                })
            }
        ...
        resume: function() {
            this.pause = false;
            cancelAnimFrame(this.requestId); (function() {
                cycleManager.cycle();
    ...
    var resizer = {
        init: function(width, height, element, desktop) {
            ...
            if (Util.isTouchScreen() || desktop) {
                DOM.on(window, "resize orientationchange",
                function() {
                    resizer.resize()
                });
                this.resize();
    ...
    function Interpolation(settings) {
        ...
        this.applyFunction = settings.applyFunction ||
        function(easing, duration, from, to, elapsed, easingParam) {
    ...
    var P = {
        ...
        playphoneAPI: "//html5sdk.galaxy.gs/PSGN.js?app_id=6388",
        pxPerPt: 20,
        highscoreKey: "remvst-was-here-again",
        totalScore: "remvst-is-bored",
    ...
    var AdsSettings = {
        ads: {
            tablet: {
                slot: "/20973361/game4_iPad_300x600",
                width: 300,
                height: 600,
                interval: 2,
                check: function() {
            ...
            mobile: {
                slot: "/20973361/game4_mobile_300x250",
                width: 300,
                height: 250,
                interval: 2,
                check: function() {
            ...
            web: {
                slot: "/20973361/game4_desktop_300x600",
                width: 300,
                height: 600,
                interval: 2,
                check: function() {
    ...
    Game.prototype = {
        ...
        handleDownEvent: function(e) {
            ...
            if (this.down) return;
            this.down = true;
            this.lastEvent = this.getPosition(e); (this.curOverlay || this.curScreen).touchStart(this.lastEvent.x, this.lastEvent.y);
            if (evtType == "touch") {}
        },
        handleMoveEvent: function(e) {
            this.lastEvent = this.getPosition(e);
            if (this.down) {
                e.preventDefault(); (this.curOverlay || this.curScreen).touchMove(this.lastEvent.x, this.lastEvent.y)
            }
        ...
        handleUpEvent: function(e) {
            if (this.down) { (this.curOverlay || this.curScreen).touchEnd(this.lastEvent.x, this.lastEvent.y);
                this.down = false;
                this.lastEvent = null
            }
            window.scrollTo(0, 1)
        },
        handleKeyDownEvent: function(e) { (this.curOverlay || this.curScreen).keyDown(e.keyCode)
        },
        handleKeyUpEvent: function(e) { (this.curOverlay || this.curScreen).keyUp(e.keyCode)
        },
        handleWheelEvent: function(e) {
            var delta = Util.limit(e.wheelDelta || -e.detail, -1, 1); (this.curOverlay || this.curScreen).mouseWheel(delta)
        },
        handleOrientationChange: function(e) {
            this.hasAccelerometer = true; (this.curOverlay || this.curScreen).orientationChange(e.alpha, e.beta, e.gamma)
        },
        menu: function() {
        ...
        openSaga: function() {
            // var url = "#";   //打开网站的叉叉， !!! LIEN
            // if (window.kik && kik.send) {
            //     kik.open(url)
            // } else if (P.cocoon) {
            //     CocoonJS.App.openURL(url)
            // } else {
            //     window.originalOpen(url)
            // }
    ...
    EndScreen.prototype = extendPrototype(Screen, {
        ...
        kik: function() {
            kik.send({
                title: "I scored " + this.score + " on Stickman Jump!",
    ...
    GameplayScreen.prototype = extendPrototype(Screen, {
        ...
        cycle: function(e) {
            ...
            if (this.elements[0] && this.elements[0].y > this.camera.y + P.height) {
                ...
                TweenPool.add(new Tween(p, "alpha", 1, 0, .2, 0,
                function() {
                    this.object.remove()
                }))
            }
    ...
    GameplayCharacter.prototype = extendPrototype(DisplayableContainer, {
        ...
        endRocket: function() {
            if (this.mode === "rocket") {
                ...
                TweenPool.add(new Tween(rocketView, "y", rocketView.y, rocketView.y - 2 * P.height, 1, 0,
                function() {
                    this.object.remove()
                }))
            }
    ...
    Star.prototype = extendPrototype(DisplayableContainer, {
        pickup: function(character) {
            ...
            TweenPool.add(new Tween(tf, "alpha", 1, 0, 1, 0,
            function() {
                this.object.remove()
            }));
    ...
    BreakingPlatform.prototype = extendPrototype(Platform, {
        characterLanded: function(character) {
            ...
            view.anchorY = -view.image.height / 2;
            this.addChild(view);
            TweenPool.add(new Tween(view, "y", 0, 200, .7, 0,
            function() {
                this.object.remove()
            }))
        }
    ...
    HybridController.prototype = extendPrototype(Controller, {
        ...
        if (!P.cocoon) { 
         ...
         //    })(window, document, "script", "//www.google-analytics.com/analytics.js", "ga")
            ...
            CocoonJS.App.loadInTheWebView("http://more.gamemix.com/cocoonoverlay.html?currentGame=stickmanjump");
            window.ga = function() {
                var args = "";
        ...
        // ga("create", "UA-52631065-1"); (function(g, m, c, d, a) {
        //     g["GameMixGA"] = a;
        //     g[a] = g[a] ||
        //     function(f) {
        //         g[a].q = g[a].q || [];
    ...
    })(window, document, "script", "./", "gmapi");
    gmapi("stickmanjump"); (function() {
        var cache = {};
        var ctx = null,
        usingWebAudio = true,
        noAudio = false;
        ...
        Howl.prototype = {
            load: function() {
                ...
                if (self._webAudio) {
                    ...
                    newNode.addEventListener("error",
                    function() {
                ...
                self._inactiveNode(function(node) {
                    ...
                    if (self._webAudio) {
                        ...
                        if (node.readyState === 4 || !node.readyState && navigator.isCocoonJS) {
                            ...
                            self._clearEndTimer(soundId); (function() {
                                var sound = self,
                                playSprite = sprite,
                                fn = callback,
                                newNode = node;
            ...
            fadeOut: function(to, len, callback, id) {
                var self = this;
                return self.fade(self._volume, to, len,
                function() {
                    if (callback) callback();
                    self.pause(id);
                    self.on("end")
