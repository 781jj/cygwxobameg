...
MyButtonBase.prototype.action = function (e) {
    if (this.tweenSize == null) {
        ...
        this.tweenSize = createjs.Tween.get(this).to({scaleX: this._scaleBegin.x * this.SCALE_ANIM, scaleY: this._scaleBegin.y * this.SCALE_ANIM}, this.TIME_ANIM, createjs.Ease.quartInOut).call(this.eventScaleBegin)
    }
};
MyButtonBase.prototype.scaleBegin = function (e) {
    this.tweenSize = createjs.Tween.get(this).to({scaleX: this._scaleBegin.x, scaleY: this._scaleBegin.y}, this.TIME_ANIM, createjs.Ease.quartInOut).call(this.eventScaleEnd)
};
...
MyButtonBase.prototype.onWork = function () {
    if (!this.hasEventListener("click"))if (this._actionDefault != null)this.on("click", this._actionDefault); else this.eventAction = this.on("click", createjs.proxy(this.action, this));
    this.cursor = "pointer"
};
...
FieldContainer.prototype.cccMouseMove = function (e) {
    if (this._mouseDown && this._step && this._moveComplete) {
        ...
        if (r > i)if (t > this._oldPos.x)this.step(1, 0); else this.step(-1, 0); else if (n > this._oldPos.y)this.step(0, 1); else this.step(0, -1);
        this._oldPos.x = t;
        this._oldPos.y = n
    }
};
...
FieldContainer.prototype.findFigures = function () {
    ...
    var t = 0;
    var n = 0;
    for (t = 0; t < this.gameManager.FIELD_WIDTH; t++)for (n = 0; n < this.gameManager.FIELD_HEIGHT; n++)if (!this.cellOnFigere(t, n))if (e.findAllCells(t, n)) {
        this.figures.push(e);
        this.addChild(e);
        var e = new FigureBlocks(this.field, this.gameManager, this)
    }
...
FieldContainer.prototype.cellOnFigere = function (e, t) {
    for (var n = 0; n < this.figures.length; n++)if (this.figures[n].isCellFigure(e, t))return true;
    return false
};
...
FieldContainer.prototype.drawOne = function (e) {
    ...
    this.addChild(r);
    for (t = -1; t < this.gameManager.FIELD_WIDTH; t++)for (n = -1; n < this.gameManager.FIELD_HEIGHT; n++) {
...
FieldContainer.prototype.getSprTile = function (e, t, n) {
    ...
    if (s != n)s = 0; else s = 1;
    if (o != n)o = 0; else o = 1;
    if (r == 0 && i == 0 && s == 0 && o == 0)return null;
...
FieldContainer.prototype.getFigureByCell = function (e, t) {
    for (var n = 0; n < this.figures.length; n++)if (this.figures[n].isCellFigure(e, t))return this.figures[n];
...
FieldContainer.prototype.step = function (e, t) {
    ...
    while (true) {
        r = false;
        for (var n = 0; n < this.figures.length; n++)if (this.figures[n].postStep())r = true;
        if (!r)break
    }
    for (var n = 0; n < this.figures.length; n++)this.figures[n].preGo();
    var i = false;
    for (n = 0; n < this.figures.length; n++)if (this.figures[n].go(e, t))i = true;
    if (i) {
...
FieldContainer.prototype.checkFigureConnection = function () {
    var e = false;
    for (var t = 0; t < this.figures.length; t++)for (var n = t + 1; n < this.figures.length; n++)if (this.figures[t].connectToFigure(this.figures[n]))e = true;
    if (e) {
        for (var t = 0; t < this.figures.length; t++)if (this.figures[t].connecting) {
...
FieldContainer.prototype.checkWin = function () {
    ...
    for (var t in e)if (e[t] > 1)return false;
    this.gameManager.setGameOver();
    console.log("WIN!!!");
    play68_submitScore(this.gameManager.levelId + 1);
    return true
};
...
FieldContainer.prototype.deleteStep = function () {
    if (this.gameManager.gui.starBar.stepNow > 0) {
        ...
        for (var t = 0; t < e.length; t++)for (var n = 0; n < e[t].length; n++)this.field[t][n] = e[t][n];
...
FigureBlocks.prototype.isCellFigure = function (e, t) {
    var n = 0;
    for (n = 0; n < this.cells.length; n++)if (this.cells[n].x == e && t == this.cells[n].y)return true;
    return false
};
...
FigureBlocks.prototype.addFigureStep = function (e) {
    if (e == null)return;
    for (var t = 0; t < this.figuresStep.length; t++)if (this.figuresStep[t].id == e.id)return;
...
FigureBlocks.prototype.postStep = function () {
    if (this.step_id == this.gameManager.STEP_CLOSE_FIGURE)for (var e = 0; e < this.figuresStep.length; e++)if (this.figuresStep[e].step_id == this.gameManager.STEP_CLOSE) {
        this.step_id = this.gameManager.STEP_CLOSE;
        return true
    }
    return false
...
FigureBlocks.prototype.go = function (e, t) {
    if (this.step_id == this.gameManager.STEP_CLOSE) {
        this.tweenMove = createjs.Tween.get(this).to({x: this.x + e * this.MOVE_CLOSE_PATH, y: this.y + t * this.MOVE_CLOSE_PATH}, this.MOVE_TIME / 2, createjs.Ease.quartInOut).to({x: this.x, y: this.y}, this.MOVE_TIME / 2, createjs.Ease.quartInOut).call(this.eventMoveComplete);
        return false
    }
    ...
    this.tweenMove = createjs.Tween.get(this).to({x: this.x + e * this.gameManager.CELL_SIZE, y: this.y + t * this.gameManager.CELL_SIZE}, this.MOVE_TIME, createjs.Ease.quartInOut).call(this.eventMoveComplete);
    return true
};
...
FigureBlocks.prototype.connectToFigure = function (e) {
    if (e.cellId == this.cellId)for (var t = 0; t < this.cells.length; t++) {
...
GameManager.prototype.loadLevel = function () {
    ...
    var s = 0;
    for (var e = 0; e < this.FIELD_WIDTH; e++)for (var t = 0; t < this.FIELD_HEIGHT; t++) {
...
MusicManager.prototype.playMusic = function () {
    if (!this._mute && this.enableSound && !this._globalMute)if (this._paused)this.musicInstance.resume(); else this.musicInstance.play({loop: -1, interrupt: createjs.Sound.INTERRUPT_ANY})
};
MusicManager.prototype.playSound = function (e, t) {
    if (!this._mute && this.enableSound && this._soundSupport && !this._globalMute)createjs.Sound.play(e, {volume: t})
};
...
PreloadState.prototype.create = function () {
    ...
    var e = [
        {src: "zibbo/Zibbo_logo_rgb-01.png", id: "zibboLogoBig"}
    ];
...
PreloadState.prototype.initAll = function (e) {
    ...
    var n = [
        {src: "imagesPack/BlockGame.png", id: "gameImg_1"},
        {src: "imagesPack/BlockGame.json", id: "gameImgJson_1", type: createjs.LoadQueue.JSON},
        {src: "text.json", id: "text_1", type: createjs.LoadQueue.JSON},
        {src: "HelpAnim/allAnim.xml", id: "gameImgXml_1", type: createjs.LoadQueue.XML},
        {src: "levels/bin/Levels.xml", id: "gameLevels_1", type: createjs.LoadQueue.XML},
        {src: "audio/music/mus_1.ogg", id: "mus_1", type: createjs.LoadQueue.SOUND},
        {src: "font/font.png", id: "fontImg_1"},
        {src: "font/font.fnt", id: "fontDataXml_1", type: createjs.LoadQueue.XML},
        {src: "audio/sound/mouse_1.ogg", id: "mouseClick", type: createjs.LoadQueue.SOUND},
        {src: "audio/sound/StarUp_2.ogg", id: "starSound", type: createjs.LoadQueue.SOUND},
        {src: "audio/sound/StarMenu.ogg", id: "winSound", type: createjs.LoadQueue.SOUND},
        {src: "audio/sound/rotate_1.ogg", id: "stepSound_1", type: createjs.LoadQueue.SOUND},
        {src: "", id: "connectSound", type: createjs.LoadQueue.SOUND}
    ];
...
PreloadState.prototype.initCookie = function () {
    if (typeof window.localStorage != "undefined")if (window.localStorage.getItem("levelComplete") == null)window.localStorage.setItem("levelComplete", 1)
};
...
GuiBlock = function (e) {
    ...
    var n = new createjs.Text("level " + (e.levelId + 1), "bold 30px Arial, Microsoft Yahei, 微软雅黑, STXihei, 华文细黑, sans-serif", "#fff");
...
LevelButton.prototype.initStar = function () {
    ...
    for (var n = 0; n < 3; n++) {
        ...
        r.y = 30;
        r.stop();
        if (n < e)r.currentAnimationFrame = 1; else r.currentAnimationFrame = 0;
        this.addChild(r)
    }
};
...
SoundButton.prototype.soundAction = function () {
    ...
    if ((new MusicManager).getMute())this.sprUp.currentAnimationFrame = 1; else this.sprUp.currentAnimationFrame = 0
};
StarBar = function (e) {
    createjs.Container.call(this);
    this.x = 60;
...
StarBar.prototype.addStep = function () {
    ...
    if (this.stepNow >= this.numStarAll)this.barMask.x = 9999; else this.barMask.x = -this.MASK_SIZE / 2 + this.MASK_SIZE * (this.stepNow / this.numStarAll)
};
...
GameOverDialog.prototype.alphaEnd = function () {
    if (this.buttonLogo)createjs.Tween.get(this.buttonLogo).to({y: 300}, 2500, createjs.Ease.elasticOut);
    var e = 157;
    var t = -65;
    var n = this.gameManager.gui.starBar.starNum;
    for (var r = 0; r < 3; r++)if (this.idAdd == r) {
        ...
        if (r < n) {
            ...
            i.scaleX = i.scaleY = 0;
            createjs.Tween.get(i).to({scaleX: 1, scaleY: 1}, 1500, createjs.Ease.elasticOut);
            (new MusicManager).playStar()
        } else {
            i.currentAnimationFrame = 1;
            i.alpha = 0;
            createjs.Tween.get(i).to({alpha: 1}, 800, createjs.Ease.quadOut)
        }
        this.addChild(i)
    }
    this.idAdd++;
    if (this.idAdd < 3)createjs.Tween.get(this).to({x: this.x}, 500, createjs.Ease.linear).call(createjs.proxy(this.alphaEnd, this))
};
HelpDialog = function (e) {
    ...
    this.finger.alpha = 0;
    this.tween = createjs.Tween.get(this.finger, {loop: true}).to({x: -this.X_END, y: this.Y_END, alpha: 1, scaleX: 1, scaleY: 1}, 500, createjs.Ease.quadOut).call(createjs.proxy(this.stepHelp_1, this)).wait(750).to({x: this.X_END}, 900, createjs.Ease.quadInOut).wait(750).call(createjs.proxy(this.stepHelp_2, this)).to({scaleX: this.SCALE_BEGIN, scaleY: this.SCALE_BEGIN, y: this.Y_END + 50, alpha: 0}, 500, createjs.Ease.quadInOut).to({x: 0, y: this.Y_BEGIN}, 1500, createjs.Ease.quadInOut)
};
...
LevelSelectMenu.prototype.drawLevelButtons = function () {
    if (this._levelButtonsContainer)this._levelButtonsContainer.removeAllChildren(); else {
    ...
    for (var o = 0; o < 5; o++)for (var u = 0; u < 4; u++) {
...
MainMenu.prototype.create = function () {
    ...
    this.addChild(r);
    r.scaleX = r.scaleY = 0;
    createjs.Tween.get(r).to({scaleX: 1, scaleY: 1}, 2500, createjs.Ease.elasticOut)
};
...
Jok.JokAnimLoader.prototype.loadAnim = function (e, t, n) {
    ...
    this.spriteSheetData = {framerate: 24, images: [n], frames: [], animations: {}};
    var s = 0;
...
Jok.JokAnimLoader.prototype.getFrameData = function (e, t) {
    var n = 0;
    while (e.frames[n]) {
        if (e.frames[n].filename == t)return{x: parseInt(e.frames[n].frame.x), y: parseInt(e.frames[n].frame.y), w: parseInt(e.frames[n].frame.w), h: parseInt(e.frames[n].frame.h)};
        n++
    }
    return null
};
Jok.JokAnimLoader.prototype.loadBitmapFont = function (e, t) {
    ...
    n[57] = "9";
    var r = 0;
    this.fontData = {images: [t], frames: [], animations: {}};
    var i = 0;
    while (true) {
        ...
        if (s) {
            ...
            if (f > 0 && l > 0) {
                ...
                this.fontData.animations[n[o]] = {frames: [i]};
                i++
            }
            r++
        } else break
